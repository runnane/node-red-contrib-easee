 ...
 **/
  ...
  module.exports = function (RED) {
    ...
    function EaseeRestClient(n) {
          ...
          try {
            switch(msg.topic){
              ...
              case "dynamic_current":
                if(!("payload" in msg) || !(typeof msg.payload == "object") || !("site_id" in msg.payload)){
                  const errorTxt = "dynamic_current failed: site_id missing";
                  node.error(errorTxt);
                }else if(!("circuit_id" in msg.payload)){
                  const errorTxt = "dynamic_current failed: circuit_id missing";
                  node.error(errorTxt);
                }else if("body" in msg.payload){
                  // Do POST update of circuit
                  node.genericCall("/sites/"+msg.payload.site_id+"/circuits/"+msg.payload.circuit_id+"/dynamicCurrent", true, "post", msg.payload.body);
                }else{
                  // GET circuit information
                  node.genericCall("/sites/"+msg.payload.site_id+"/circuits/"+msg.payload.circuit_id+"/dynamicCurrent");
                }
      ...
      node.doAuthRestCall = async (url, method="get", headers = {}, body = {}) => {
    ...
    try{
          ...
          //   return json;
            ...
            node.error({url: node.connectionConfig.RestApipath + url,   method: method,
              headers: headers,
              body:(method=="post")?bodyPayload:null
            });
